...
class CNNPretrainedModel(nn.Module, LightningTemplateModel):
    """
...
class IITPetClassification(pl.LightningModule):
    ...
    def __init__(self, path, transforms, params):
        ...
        img_names = set(
            [re.findall(IMG_NAME, x.name.lower())[0] for x in self.path.ls() if x.name.lower().endswith('.jpg')])
    ...
    def training_step(self, batch, batch_idx):
        if self.trainer.batch_idx == 0:
            if self.trainer.current_epoch == self.params['stages'][0]['epochs']:
                ...
                self.trainer.lr_schedulers[0]['scheduler'] = self.create_lr_scheduler(self.params['stages'][1],
                ...
                self.trainer.lr_schedulers[0]['scheduler'] = self.create_lr_scheduler(self.params['stages'][2],
                                                                                      self.trainer.optimizers[0])
...
if __name__ == "__main__":
    ...
    trainer = Trainer(max_epochs=12, gpus=1, callbacks=[lr_logger])
    trainer.fit(system)
